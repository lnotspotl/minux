...
// Data structure for keeping track of number of calls and total execution time
struct syscallstats {
	int count[NUM_SYSCALLS];
	unsigned long total_time[NUM_SYSCALLS];
};

struct perprocesssyscallstats {
	struct syscallstats stats[NPROC];
	Bool enabled;
};

extern struct perprocesssyscallstats syscall_statistician;
