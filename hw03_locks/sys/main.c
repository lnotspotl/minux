...
char output2[10];
int count2;
void reader2 (char msg, int lck, int max_priority)
{
        int     ret;

        kprintf ("  %c: to acquire lock\n", msg);
        lock (lck, READ, max_priority);
        output2[count2++]=msg;
...
void writer2 (char msg, int lck, int max_priority)
{
	kprintf ("  %c: to acquire lock\n", msg);
        lock (lck, WRITE, max_priority);
        output2[count2++]=msg;
        kprintf ("  %c: acquired lock, sleep 3s\n", msg);
        sleep (3);
        output2[count2++]=msg;
...
void test2 ()
{
	...
	rd1 = create(reader2, 2000, 20, "A-reader", 3, 'A', lck, 20);
	rd2 = create(reader2, 2000, 20, "B-reader", 3, 'B', lck, 30);
	rd3 = create(reader2, 2000, 20, "D-reader", 3, 'D', lck, 25);
	rd4 = create(reader2, 2000, 20, "E-reader", 3, 'E', lck, 20);
        wr1 = create(writer2, 2000, 20, "C-writer", 3, 'C', lck, 28);
...
{
        kprintf ("  %s: to acquire lock\n", msg);
        lock (lck, LOCK_WRITE, DEFAULT_LOCK_PRIO);
...
void test3 ()
{
        ...
        kprintf("lck2: %d, lck3: %d\n", lck2, lck3);

        lock(lck2, LOCK_READ, DEFAULT_LOCK_PRIO);
        lock(lck3, LOCK_READ, DEFAULT_LOCK_PRIO);

        int release_result = releaseall(2, lck2, lck3);
        assert(release_result == OK, "Test 3 failed");

        rd1 = create(reader3, 2000, 25, "reader A", 2, "reader A", lck);
        rd2 = create(reader3, 2000, 30, "reader B", 2, "reader B", lck);
        wr1 = create(writer3, 2000, 20, "writer", 2, "writer", lck);
